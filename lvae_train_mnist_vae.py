...
parser.add_argument('--latent_dim', type=int, default=2, help='Latent feature size')
...
def train(args, lvae):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            loss, mu, output, output_mu, x_re, rec, kl, ce = lvae.loss(data, target, target_en, next(beta), lamda)
...
# val
        if epoch % args.val_interval == 0 and epoch >= 0:
            ...
            for data_val, target_val in val_loader:
                ...
                loss_val, mu_val, output_val, output_mu_val, val_re, rec_val, kl_val, ce_val = lvae.loss(data_val, target_val, target_val_en, next(beta), lamda)
...
# omn_test
                print('Saving model...')
                torch.save(lvae.state_dict(), 'lvae%d/model.pt' % args.lamda)
